// src/pages/student/course-details/index.jsx
...
import {
  // checkCoursePurchaseInfoService, // optional redirect if already purchased
...
function StudentViewCourseDetailsPage() {
  ...
  }, [auth?.authenticate, auth?.user, navigate, studentViewCourseDetails]);

  // Open preview dialog when a preview URL is set
  useEffect(() => {
    if (displayCurrentVideoFreePreview) setShowFreePreviewDialog(true);
  }, [displayCurrentVideoFreePreview]);

  // Fetch details when course id is set
  useEffect(() => {
  ...
  // Set current course id from route
  useEffect(() => {
  ...
  // Redirect to PayPal approval URL when ready
  useEffect(() => {
    if (approvalUrl) {
                ...
                {curriculum.map((item, index) => {
                  ...
                  return (
                    <li
                      ...
                      onClick={
                        clickable ? () => handleSetFreePreview(item) : undefined
                      }
                    >
                      {clickable ? (
          ...
          <Card className="sticky top-4">
            <CardContent className="p-6 space-y-4">
              ...
              <Button onClick={handleCreatePayment} className="w-full" disabled={creatingPayment}>
                {creatingPayment ? "Redirecting..." : "Buy Now"}
              </Button>
            </CardContent>
          </Card>
        </aside>
